...
router.post("/", async (req, res) => {
  ...
  if (!VALID_FORMATS.includes(image_format.toUpperCase())) {
    return res.status(400).json({
      error: `Invalid image format. Supported formats: ${VALID_FORMATS.join(", ")}`,
    });
  }

  const width = Math.min(Math.max(parseInt(image_width) || 500, 100), 2000);

  try {
    const response = await axios.post(
      `${process.env.API_URL}?access-token=${process.env.ACCESS_TOKEN}`,
      {
        qr_code_text,
    ...
    const mimeType =
      image_format.toLowerCase() === "svg"
        ? "image/svg+xml"
        : `image/${image_format.toLowerCase()}`;
